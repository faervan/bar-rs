...
use smithay_client_toolkit::{delegate_output, delegate_registry, output::{OutputHandler, OutputInfo, OutputState}, reexports::client::{globals::registry_queue_init, protocol::wl_output::{self, WlOutput}, Connection, QueueHandle}, registry::{ProvidesRegistryState, RegistryState}, registry_handlers};

pub fn get_wl_output(name: &String) -> Result<WlOutput, Box<dyn Error>> {
    ...
    let mut outputs = list_outputs.output_state.outputs();
    ...
    Ok(outputs.find(|o| list_outputs.output_state.info(o).unwrap().name.as_ref() == Some(name)).unwrap())
