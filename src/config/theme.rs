...
impl<'de> Deserialize<'de> for BarTheme {
    ...
    {
        let mut map: Map<String, Value> = config::Map::deserialize(deserializer)?;

        let get_palette = |k: &str, map: &mut Map<_, _>| {
            map.remove(k)
                .and_then(|v| {
                    Palette::deserialize(v)
                        .inspect_err(|e: &ConfigError| {
...
impl<'de> Deserialize<'de> for Palette {
    ...
    where
        D: serde::Deserializer<'de>,
    {
        let map: HashMap<String, Value> = config::Map::deserialize(deserializer)?;
...
fn get_color(key: &str, map: &Map<String, Value>) -> Option<Color> {
...
impl From<BarTheme> for ValueKind {
...
impl From<Palette> for ValueKind {
    fn from(value: Palette) -> Self {
        let map = Map::from([
            (
                String::from("background"),
                value.background.map(|c| color_to_value(c)).into(),
            ),
            (
                String::from("text"),
                value.text.map(|c| color_to_value(c)).into(),
            ),
            (
                String::from("icon"),
                value.icon.map(|c| color_to_value(c)).into(),
            ),
        ]);
        ValueKind::Table(map)
    }
...
fn color_to_value(c: Color) -> ValueKind {
