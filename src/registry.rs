...
impl Registry {
    pub fn register_module<T: Builder>(&mut self) where T::Output: Module {
        let output = T::build();
    ...
    pub fn register_listener<T: Builder>(&mut self) where T::Output: Listener {
        let output = T::build();
    ...
    pub fn get_listeners<'a>(&'a self, enabled: &'a HashSet<TypeId>) -> impl Iterator<Item = &'a Box<dyn Listener>> {
        enabled.iter()
            .map(|id|
    ...
    pub fn enabled_listeners<'a>(&'a self, modules: &'a EnabledModules) -> impl Iterator<Item = TypeId> + 'a {
        modules.get_all()
            .filter_map(|m|
                self.module_names
                    .get(m)
                    .map_none(|| if !m.is_empty() {
    ...
    pub fn all_listeners<'a>(&'a self) -> impl Iterator<Item = (&'a TypeId, &'a Box<dyn Listener>)> {
        self.listeners.iter()
    }
}
