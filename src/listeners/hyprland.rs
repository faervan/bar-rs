...
impl Listener for HyprListener {
    fn config(&self) -> Vec<ConfigEntry> {
        vec![ConfigEntry::new("general", "close_on_fullscreen", true)]
    }
            ...
            stream::channel(1, |mut sender| async move {
                ...
                sender
                    .send(Message::update(Box::new(move |reg| {
                        ...
                        hypr.fullscreen = workspaces.1[workspaces.0].1;
                ...
                listener.add_active_monitor_changed_handler(move |data| {
                    ...
                    Box::pin(async move {
                        sender
                            .send(Message::update(Box::new(move |reg| {
                                reg.get_listener_mut::<HyprListener>().active_monitor =
                                    data.monitor_name
                            })))
                            .await
                            .unwrap_or_else(|err| {
                                eprintln!(
                                    "Trying to send fullscreen toggle event failed with err: {err}"
                                );
                            });
                    })
                });
                ...
                listener.add_workspace_changed_handler(move |data| {
                    ...
                    Box::pin(async move {
                        ...
                        sender
                            .send(Message::Perform(|reg, config, open| {
                                let monitor_focused =
                                    reg.get_listener::<HyprListener>().active_monitor
                                        == config.monitor;
                    ...
                    Box::pin(async move {
                        ...
                        sender
                            .send(Message::Perform(|reg, config, open| {
                                ...
                                workspaces.open[workspaces.active].1 =
                                    !workspaces.open[workspaces.active].1;
                ...
                listener.add_config_reloaded_handler(move || {
                    ...
                    Box::pin(async move { reserve_bar_space(&monitor) })
                });
